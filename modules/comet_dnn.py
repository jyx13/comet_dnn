...
def loss(predictions, labels):
    ...
    for i in range(FLAGS.num_classes):
        ...
        tf.summary.histogram(name+"_res", residuals[:, i])
        # Fill scalar summaries of the mean residuals and the RMS
        tf.summary.scalar(name+"_mean_res", mean_residuals[i])
        tf.summary.scalar(name+"_rms", root_mean_square[i])
